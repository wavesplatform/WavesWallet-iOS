...
final class MarketPulseWidgetViewController: UIViewController {
    ...
    @IBAction private func settingsTapped(_: Any) {
    ...
    @IBAction private func updateTapped(_: Any) {
        WidgetAnalyticManager.shared.trackEvent(.widgets(.marketPulseActive))
    ...
    @IBAction private func changeCurrency(_: Any) {
        if currency == .usd {
            currency = .eur
        } else {
            currency = .usd
...
extension MarketPulseWidgetViewController: NCWidgetProviding {
    ...
    func widgetActiveDisplayModeDidChange(_: NCWidgetDisplayMode, withMaximumSize _: CGSize) {
        updatePrefferedSize()
    }
