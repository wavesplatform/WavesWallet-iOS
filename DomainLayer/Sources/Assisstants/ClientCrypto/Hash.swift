...
public class Hash {
    ...
    public static func secureHash(_ input: [UInt8]) -> [UInt8] {
    ...
    public static func fastHash(_ input: [UInt8]) -> [UInt8] {
        var res = Data(count: 32)
        var key: UInt8 = 0
        res.withUnsafeMutableBytes {(bytes: UnsafeMutablePointer<UInt8>)->Void in
            crypto_generichash_blake2b(bytes, 32, input, UInt64(input.count), &key, 0)
    ...
    public static func sign(_ input: [UInt8], _ key: [UInt8]) -> [UInt8] {
