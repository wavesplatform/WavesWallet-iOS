    ...
    private func reduce(state: DexMarket.State, event: DexMarket.Event) -> DexMarket.State {
        ...
        case .tapInfoButton(let index):
            ...
            if let pair = state.section.items[index].pair {
                ...
                let infoPair = DexInfoPair.DTO.Pair(amountAsset: pair.amountAsset.id, amountAssetName: pair.amountAsset.name, priceAsset: pair.priceAsset.id, priceAssetName: pair.priceAsset.name, isPopular: !pair.isHidden)
