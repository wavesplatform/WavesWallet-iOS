...
extension InvestmentSection {
    static func map(from assets: [DomainLayer.DTO.SmartAssetBalance]) -> [InvestmentSection] {
        let generalItems = assets
            .filter { $0.asset.isSpam != true && $0.settings.isHidden != true }
            .map { InvestmentRow.asset($0) }

        let generalSection: InvestmentSection = .init(kind: .general,
                                                                  items: generalItems,
                                                                  isExpanded: true)
        ...
        var sections: [InvestmentSection] = [InvestmentSection]()
        sections.append(.init(kind: .search, items: [.search], isExpanded: true))
        sections.append(generalSection)

        if !hiddenItems.isEmpty {
            let hiddenSection: InvestmentSection = .init(kind: .hidden(count: hiddenItems.count),
                                                                     items: hiddenItems,
                                                                     isExpanded: false)
        ...
        if !spamItems.isEmpty {
            let spamSection: InvestmentSection = .init(kind: .spam(count: spamItems.count),
                                                                   items: spamItems,
                                                                   isExpanded: false)
