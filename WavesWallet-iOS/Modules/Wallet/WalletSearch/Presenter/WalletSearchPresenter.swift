...
private extension WalletSearch.State {
    static func initialState(assets: [DomainLayer.DTO.SmartAssetBalance]) -> WalletSearch.State {
        return WalletSearch.State(assets: assets,
                                  sections: WalletSearch.ViewModel.Section
                                      .map(from: assets, searchString: "", includeSpam: false),
                                  action: .none)
    }
}

extension WalletSearch.ViewModel.Section {
    static func map(from assets: [DomainLayer.DTO.SmartAssetBalance], searchString: String,
                    includeSpam: Bool) -> [WalletSearch.ViewModel.Section] {
        ...
        if !spamItems.isEmpty, includeSpam {
