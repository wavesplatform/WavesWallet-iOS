...
extension AssetDetailCell: ViewConfiguration {
    func update(with model: AssetDetailTypes.DTO.Asset.Info) {
        ...
        viewIssuer.isHidden = model.issuer.isEmpty
        viewDescription.isHidden = model.description.isEmpty
    }
}
...
extension AssetDetailCell: ViewCalculateHeight {
    static func viewHeight(model: AssetDetailTypes.DTO.Asset.Info, width: CGFloat) -> CGFloat {
        ...
        let nameHeight = model.name
            .maxHeightMultiline(font: UIFont.systemFont(ofSize: 13), forWidth: width - Constants.padding * 2)
        offset += nameHeight
        offset += Constants.titleBlockOffset

        if !model.issuer.isEmpty {
            ...
            let issuerHeight = model.issuer.maxHeightMultiline(font: UIFont.systemFont(ofSize: 13),
                                                               forWidth: width - Constants.padding * 2 - Constants
                                                                   .pictureSize - Constants.pictureLeftPadding)
            offset += issuerHeight
            offset += Constants.titleBlockOffset
        }
        ...
        let idHeight = model.id.maxHeightMultiline(font: UIFont.systemFont(ofSize: 13),
                                                   forWidth: width - Constants.padding * 2 - Constants.pictureSize - Constants
                                                       .pictureLeftPadding)
        offset += idHeight
        offset += Constants.titleBlockOffset
        ...
        if !model.description.isEmpty {
            ...
            offset += Constants.titleDefaultHeight
            offset += Constants.titleOffset
            let descriptionHeight = model.description
                .maxHeightMultiline(font: UIFont.systemFont(ofSize: 13), forWidth: width - Constants.padding * 2)
