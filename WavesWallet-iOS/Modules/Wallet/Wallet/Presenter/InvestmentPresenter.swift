...
final class InvestmentPresenter: InvestmentPresenterProtocol {
    ...
    private var assetListener: Signal<InvestmentEvent>?
    ...
    private func queryCleanWalletBanner() -> Feedback {
        return react(request: { (state) -> Bool? in
            state.isNeedCleanWalletBanner ? true : nil

        }, effects: { [weak self] _ -> Signal<InvestmentEvent> in

            guard let self = self else { return Signal.empty() }
            return self.interactor.setCleanWalletBanner().map { .completeCleanWalletBanner($0) }
    ...
    private func queryCleanWallet() -> Feedback {
        return react(request: { (_) -> Bool? in
            ...
            guard let self = self else { return Signal.empty() }
            return self.interactor.isShowCleanWalletBanner().map { .isShowCleanWalletBanner($0) }
                .asSignal(onErrorSignalWith: Signal.empty())
        })
    }
    ...
    private func reduce(state: inout InvestmentState, event: InvestmentEvent) {
        ...
        case let .tapRow(indexPath):
            ...
            case .general:
                guard let asset = section.items[indexPath.row].asset else { return }
                moduleOutput?
                    .showAsset(with: asset,
                               assets: state.assets.filter { $0.asset.isSpam != true && $0.settings.isHidden != true })
...
//
        ...
        case let .isShowCleanWalletBanner(isShowCleanWalletBanner):
