...
final class ServerMaintenanceViewController: UIViewController {
    ...
    weak var delegate: ServerMaintenanceViewControllerDelegate?
    ...
    @IBAction private func retryTapped(_ sender: Any) {
        ...
        developmentConfigsRepository
            .isEnabledMaintenance()
            .observeOn(MainScheduler.instance)
            .subscribe(onNext: { [weak self] in self?.validateMaintenanceStatus(isEnabledMaintenance: $0) },
                       onError: { [weak self] _ in self?.setupErrorState() })
            .disposed(by: disposeBag)
    }
