...
final class DexCreateOrderInteractor: DexCreateOrderInteractorProtocol {
    ...
    func getFee(amountAsset: String,
        ...
        auth.authorizedWallet().flatMap { [weak self] wallet -> Observable<DexCreateOrder.DTO.FeeSettings> in
            ...
            return Observable.zip(strongSelf.orderBookInteractor.orderSettingsFee(),
                                  strongSelf.transactionsRepository.feeRules(),
                                  isSmartAddress,
                                  strongSelf.accountBalance.balances(),
                                  isSmartAssets,
                                  strongSelf.obtainWavesAsset(accountAddress: wallet.address))
                .flatMap { [weak self]
                    (settingOrderFee, feeRules, isSmartAddress, availableAccountBalances, isSmartAssets, wavesAsset) -> Observable<DexCreateOrder.DTO.FeeSettings> in
    ...
    private func filterFeeAssets(feeAsset: DomainLayer.DTO.Dex.SmartSettingsOrderFee.FeeAsset,
                                 wavesAsset: Asset,
    ...
    private func calculateFee(isSmartAddress: Bool,
                              ...
                              feeAssetId: String,
                              asset: Asset,
                              settingOrderFee: DomainLayer.DTO.Dex.SmartSettingsOrderFee.FeeAsset,
                              baseFee: Int64,
        ...
        let assetDecimal = settingOrderFee.asset.precision
    ...
    func calculateMarketOrderPrice(amountAsset: Asset,
        ...
        let zeroPriceValue = Money(0, priceAsset.precision)
                ...
                .flatMap { [weak self] serverEnvironment -> Observable<DexCreateOrder.DTO.MarketOrder> in
                    ...
                    return self.orderBookRepository
                        ...
                        .flatMap { orderBook -> Observable<DexCreateOrder.DTO.MarketOrder> in
                            ...
                            var filledAmount: Money = Money(0, amountAsset.precision)
                            ...
                            for askOrBid in bidOrAsks {
                                ...
                                askOrBidPrice = Money
                                    .price(amount: askOrBid.price, amountDecimals: amountAsset.precision,
                                           priceDecimals: priceAsset.precision)

                                let askOrBidAmount = Money(askOrBid.amount, amountAsset.precision)
                            ...
                            let priceAvg = filledAmount
                                .decimalValue > 0 ? Money(value: computedTotal.decimalValue / filledAmount.decimalValue,
                                                          priceAsset.precision) : zeroPriceValue
            ...
            .flatMap { [weak self] wallet, serverEnvironment -> Observable<ResponseType<DexCreateOrder.DTO.Output>> in
                ...
                return matcher
                    .flatMap { [weak self] matcherPublicKey -> Observable<ResponseType<DexCreateOrder.DTO.Output>> in
                        ...
                        let precisionDifference = (order.priceAsset.precision - order.amountAsset.precision) + Constants
