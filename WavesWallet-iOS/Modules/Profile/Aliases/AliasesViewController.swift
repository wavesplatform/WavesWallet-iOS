...
final class AliasesViewController: UIViewController {
    ...
    private lazy var tapGesture = UITapGestureRecognizer(target: self,
                                                         action: #selector(handlerTapGesture(tap:)))
    ...
    override func viewDidLoad() {
        ...
        tableView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: Constants.bottomPadding + Constants.topBarHeight, right: 0)
    ...
    private func needShowView() {
        if isHiddenInfoView {
            eventInput.onNext(.showCreateAlias)
            showInfoView()
        } else {
            eventInput.onNext(.hideCreateAlias)
            hideInfoView()
        }
    }
    ...
    private func showInfoView() {
        ...
        tableView.contentInset = UIEdgeInsets(top: 0, left: 0, bottom: Constants.bottomPadding + size.height, right: 0)
    ...
    @objc func handlerTapGesture(tap _: UITapGestureRecognizer) {
        needShowView()
    }
}
...
extension AliasesViewController: UIGestureRecognizerDelegate {
    func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        let frame = CGRect(x: aliasesInfoView.frame.origin.x, y: aliasesInfoView.frame.origin.y,
                           width: aliasesInfoView.frame.width, height: Constants.topBarHeight)
