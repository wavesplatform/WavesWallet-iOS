...
public extension NSAttributedString {
    class func styleForBalance(text: String, font: UIFont, weight: UIFont.Weight = .semibold) -> NSAttributedString {
        ...
        let attributes = [NSAttributedString.Key.font: UIFont.systemFont(ofSize: font.pointSize, weight: weight)]
        let attrString = NSMutableAttributedString(string: text, attributes: attributes)

        if range.location != NSNotFound {
            let length = text.count - range.location
            attrString.addAttributes([NSAttributedString.Key.font: UIFont.systemFont(ofSize: font.pointSize, weight: .regular)],
